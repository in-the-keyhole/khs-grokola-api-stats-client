...
public class ApiFilter implements Filter {
	...
	private final static Logger LOG = Logger.getLogger(ApiPublishThread.class.getName());
	...
	public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain)
		...
		if (thread == null && this.valid) {
			...
			thread.setToken(this.token);
			thread.setThreshold(this.watchThreshold);
			thread.setSleep(this.sleep);
			Thread t = new Thread(thread);
			t.setDaemon(true);
			t.start();
			LOG.info("API Watch Thread Started, will publish API's for every " + this.watchThreshold
					+ " encounters... ");
		}
